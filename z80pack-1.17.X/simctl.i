...
# 45 "../z80sim/simctl.c" 2
# 1 "/Applications/microchip/mplabc32/v2.01/bin/../lib/gcc/pic32mx/4.5.1/../../../../pic32mx/include/signal.h" 1 3
# 103 "/Applications/microchip/mplabc32/v2.01/bin/../lib/gcc/pic32mx/4.5.1/../../../../pic32mx/include/signal.h" 3
typedef int sig_atomic_t;

extern void (* signal(int, void (*)(int)))(int);
extern int raise(int);
# 46 "../z80sim/simctl.c" 2
# 1 "../z80sim/sim.h" 1
# 104 "../z80sim/sim.h"
...
static void do_go(char *s)
{
 while (((_ctype+1)[(unsigned)(int)*s] & (0x08|0x80)))
  s++;
 if (((_ctype+1)[(unsigned)(int)*s] & (0x04|0x40)))
  PC = ram + exatoi(s);
 cont:
 cpu_state = 1;
 cpu_error = 0;
...
static void do_modify(char *s)
{
 ...
 for (;;) {
  ...
  if (!((_ctype+1)[(unsigned)(int)nv[0]] & (0x04|0x40)))
   break;
  *wrk_ram++ = exatoi(nv);
  if (wrk_ram > ram + 65535)
   wrk_ram = ram;
...
static void do_port(char *s)
{
 ...
 fgets(nv, sizeof(nv), (&_iob[0]));
 if (((_ctype+1)[(unsigned)(int)*nv] & (0x04|0x40)))
...
static void do_reg(char *s)
{
 ...
 if (*s == '\0') {
  ...
  if (strncmp(s, "bc'", 3) == 0) {
   ...
   printf("S-FLAG = %c : ", (F & 128) ? '1' : '0');
   fgets(nv, sizeof(nv), (&_iob[0]));
   F = (exatoi(nv)) ? (F | 128) : (F & ~128);
  } else if (strncmp(s, "fz", 2) == 0) {
   printf("Z-FLAG = %c : ", (F & 64) ? '1' : '0');
   fgets(nv, sizeof(nv), (&_iob[0]));
   F = (exatoi(nv)) ? (F | 64) : (F & ~64);
  } else if (strncmp(s, "fh", 2) == 0) {
   printf("H-FLAG = %c : ", (F & 16) ? '1' : '0');
   fgets(nv, sizeof(nv), (&_iob[0]));
   F = (exatoi(nv)) ? (F | 16) : (F & ~16);
  } else if (strncmp(s, "fp", 2) == 0) {
   printf("P-FLAG = %c : ", (F & 4) ? '1' : '0');
   fgets(nv, sizeof(nv), (&_iob[0]));
   F = (exatoi(nv)) ? (F | 4) : (F & ~4);
  } else if (strncmp(s, "fn", 2) == 0) {
   printf("N-FLAG = %c : ", (F & 2) ? '1' : '0');
   fgets(nv, sizeof(nv), (&_iob[0]));
   F = (exatoi(nv)) ? (F | 2) : (F & ~2);
  } else if (strncmp(s, "fc", 2) == 0) {
   printf("C-FLAG = %c : ", (F & 1) ? '1' : '0');
   fgets(nv, sizeof(nv), (&_iob[0]));
   F = (exatoi(nv)) ? (F | 1) : (F & ~1);
 ...
 *(ram + soft[i].sb_adr) = 0x76;
 while (!((_ctype+1)[(unsigned)(int)*s] & (0x20)) && !((_ctype+1)[(unsigned)(int)*s] & (0x10)))
  s++;
 if (*s != ',')
  soft[i].sb_pass = 1;
 else
...
static void do_hist(char *s)
{
 ...
 switch (*s) {
 case 'c':
  memset((char *) his, 0, sizeof(struct history) * 100);
  h_next = 0;
  h_flag = 0;
  break;
 default:
